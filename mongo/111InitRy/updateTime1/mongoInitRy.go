...
//ObjectId("616e4062dc5a6346559bc07a")
func shell(mongodb *mgo.Session, database string, skip, limit int) (err error) {
	...
	c.Find(bson.M{"_id": bson.M{"$gt": bson.ObjectIdHex("616e4062dc5a6346559bc07a")}}).Select(bson.M{"package": 1, "updateTime": 1, "language": 1}).Skip(skip).Limit(limit).All(&mongoAppInfos)
...
func insertDB(s MongoAppInfo, lang string, apkId int64) error {
	...
	apkDescSql := "update oz_apk_desc  set description = ?, png_icon_id = ?,jpg_icon_id = ? where apk_id = ?,language = ?"
	_, err = DB.Exec(apkDescSql, v.DescriptionHtml, iconId, iconId, apkId, lang)
