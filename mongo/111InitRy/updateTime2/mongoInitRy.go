...
//ObjectId("61772b0b275289742a50b587")
func shell(mongodb *mgo.Session, database string, skip, limit int) (err error) {
	...
	c.Find(bson.M{"_id": bson.M{"$gt": bson.ObjectIdHex("61772b0b275289742a50b587")}}).Select(bson.M{"package": 1, "updateTime": 1, "language": 1}).Skip(skip).Limit(limit).All(&mongoAppInfos)
...
func insertDB(s MongoAppInfo, lang string, apkId int64) error {
	...
	apkDescSql := "update oz_apk_desc  set description = ?, png_icon_id = ?,jpg_icon_id = ? where apk_id = ?"
	_, err = DB.Exec(apkDescSql, v.DescriptionHtml, iconId, iconId, apkId)
