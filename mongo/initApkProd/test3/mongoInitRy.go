...
//ObjectId("6197b9ca685adbd671db4750")
func shell(mongodb *mgo.Session, database string, skip, limit int) (err error) {
	...
	c.Find(bson.M{"_id": bson.M{"$gt": bson.ObjectIdHex("6197b9ca685adbd671db4750")}}).Sort("_id").Skip(skip).Limit(limit).All(&mongoAppInfos)
