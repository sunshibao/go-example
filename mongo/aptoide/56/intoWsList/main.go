...
func main() {
	...
	var err2 error
	for {
		if err2 == nil && skip < 700000 {
			skip = 0 + s*25
			err2 = GetListAll(skip)
			s++
		} else {
			break
		}
...
func GetListAll(skip int) error {
	url := fmt.Sprintf("https://ws75.aptoide.com/api/7/listApps/store_name=honor-api/offset=%d", skip)
	resp, _ := http.Get(url)
	defer resp.Body.Close()
	body, _ := ioutil.ReadAll(resp.Body)
	var ws80 Ws80
	json.Unmarshal([]byte(string(body)), &ws80)
	insertWsData(ws80.Datalist)
...
func insertWsData(dataList Datalist) (err error) {
	for _, s := range dataList.List {
		fmt.Println("packageName:", s.Package)
		advertising := 0
		billing := 0
