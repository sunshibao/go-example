...
func start(minId int) {
	//建立连接
	NewCosClient()
	uri := "root:Droi*#2021@tcp(127.0.0.1:3306)/ry_market_examine?charset=utf8&parseTime=True&loc=Local"
...
type ApkInfo struct {
	...
	JpgIconUrl string `db:"hd_image_url"`
}
...
func GetApkList(minId, skip, limit int) (err error) {
	...
	sql3 := "select hd_image_url from oz_image limit ?,? "
	err = DB.Get(&hdImageUrl, sql3, skip, limit)

	if hdImageUrl == "" {
		return err
	}
	log.Println("hd_image_url" + gconv.String(skip))
	resultIcon := CheckImgUrl(hdImageUrl)
	if !resultIcon {
		NewUploadImg(hdImageUrl)
	}
	return nil
}
...
func CheckImgUrl(filePath string) bool {
	...
	if err != nil {
		...
		delSql := "insert into  img_url_new (imgUrl) values(?)"
