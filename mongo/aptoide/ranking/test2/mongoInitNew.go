...
//10万
//ObjectId("6147c3f466de4b663ab88397")
func shell(mongodb *mgo.Session, database string, skip, limit int) (err error) {
	...
	c.Find(bson.M{"_id": bson.M{"$gt": bson.ObjectIdHex("6147c3f466de4b663ab88397")}}).Select(bson.M{"package_name": 1, "maxInstalls": 1}).Sort("_id").Skip(skip).Limit(limit).All(&mongoAppInfos)
