...
//10万
//ObjectId("6177d39c275289742a69f0e0")
func shell(mongodb *mgo.Session, database string, skip, limit int) (err error) {
	...
	c.Find(bson.M{"_id": bson.M{"$gt": bson.ObjectIdHex("6177d39c275289742a69f0e0")}}).Select(bson.M{"package_name": 1, "maxInstalls": 1}).Sort("_id").Skip(skip).Limit(limit).All(&mongoAppInfos)
