...
func GetApkList(skip, limit int) (err error) {
	sql1 := "select package_name from 01temp limit ?,? "
	...
	if err != nil {
		...
		for rows.Next() {
			...
			fmt.Println("package:", packageName, "--------skip:", skip)
			GetApkDetail(packageName)
		}
	}
	return nil
}

func GetApkDetail(packageName string) (err error) {

	url := fmt.Sprintf("https://ws75.aptoide.com/api/7/getApp/store_name=honor-api/package_name=%s", packageName)
...
func insertWsData(nodes Nodes) (err error) {
	...
	age := strings.Trim(newData.Age.Pegi, "PEGI-")
	...
	DB.Table("01temp_detail").Create(newMysql)
	return nil
}
