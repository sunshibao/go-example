...
func insertDB(s MongoAppInfo, lang string) error {
	...
	apkSql := "insert into oz_apk (package_name,apk_name,version_code,version_name,download_url,company,file_size,download_num,apk_res_type,apk_type,status)values(?,?,?,?,?,?,?,?,?,?,?)"
	appResult, err := DB.Exec(apkSql, s.Package, v.Title, appDetails.VersionCode, appDetails.VersionString, downloadUrl, appDetails.DeveloperName, appDetails.InfoDownloadSize.Low, appDetails.DownloadCount.Low, appDetails.CategoryName, apkType, statusApk)
