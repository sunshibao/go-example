...
//130万
//ObjectId("61753384275289742a1c3b2b")
//ObjectId("61755372275289742a211b8e")   到这里了
func shell(mongodb *mgo.Session, database string, skip, limit int) (err error) {
	...
	c.Find(bson.M{"_id": bson.M{"$gt": bson.ObjectIdHex("61755372275289742a211b8e")}}).Sort("_id").Skip(skip).Limit(limit).All(&mongoAppInfos)
