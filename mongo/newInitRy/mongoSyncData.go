...
type MongoAppInfo struct {
	...
	Installs            int64    `bson:"maxInstalls"`
...
func shell(mongodb *mgo.Session, database string, skip, limit int) (err error) {
	...
	for k, v := range mongoAppInfos {
		...
		apkSql := "insert into oz_apk (package_name,apk_name,version_code,version_name,download_url,company,file_size,download_num,apk_res_type,status)values(?,?,?,?,?,?,?,?,?,?)"
		appResult, err := DB.Exec(apkSql, v.PackageName, v.Title, v.CheckVersionCode, v.CheckVersionName, downloadUrl, v.Developer, v.CheckSize, v.Installs, strings.ToLower(v.GenreId), statusApk)
		...
		apkDescSql := "insert into oz_apk_desc (apk_id,package_name,apk_name_lang,description,app_permission_desc,app_permission_url,ver_upt_des,language,png_icon_id,jpg_icon_id,status)values(?,?,?,?,?,?,?,?,?,?,?)"
